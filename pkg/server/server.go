...
func NewTunnelServer(cfg *config.ServerConfig) *TunnelServer {
	...
	// Rate limiter: 5 requests per minute, burst of 5 per IP
...
func (s *TunnelServer) handleControlConnection(conn net.Conn) {
	...
	if err == nil {
		...
		if !limiter.Allow() {
			s.logger.Warn("Rate limit exceeded for IP", slog.String("ip", ip))
	...
	// 1. Generate secure 32-byte random challenge nonce
	nonce := make([]byte, 32)
	...
	// Remove authentication deadline for long-lived multiplexing session
	_ = conn.SetDeadline(time.Time{})

	// 2. Map client configuration by ClientID
	s.mu.Lock()
