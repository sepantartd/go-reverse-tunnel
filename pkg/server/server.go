...
func (s *TunnelServer) FindFreeDynamicPort() (int, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	return 0, fmt.Errorf("no free dynamic port available in range %d-%d", minPort, maxPort)
}
...
func (s *TunnelServer) Start() error {
	...
	if s.config.EnableAutoTLS {
		...
		s.logger.Info("Auto-TLS configured with Let's Encrypt", slog.String("domain", s.config.AutoTLSDomain))
	...
	if s.config.DashboardAddr != "" {
		go func() {
			...
			s.logger.Info("Dashboard, Metrics, and pprof Profiler listening", slog.String("addr", s.config.DashboardAddr))
...
func (s *TunnelServer) handleControlConnection(conn net.Conn) {
	...
	if s.tlsConfig != nil {
		...
		if err := tlsConn.Handshake(); err != nil {
			s.logger.Warn("TLS handshake failed", slog.String("remote_addr", conn.RemoteAddr().String()), slog.String("error", err.Error()))
			return
		}
		conn = tlsConn
	}
	...
	if !hmac.Equal([]byte(clientHMAC), []byte(expectedHMAC)) {
		...
		resp, _ := json.Marshal(AuthResponse{Status: "error", Error: "Authentication failed"})
	...
	if assignedClient == nil && len(s.config.Clients) > 0 {
		...
		s.mu.Unlock()
		resp, _ := json.Marshal(AuthResponse{Status: "error", Error: "Client ID not allowed"})
		_, _ = conn.Write(append(resp, '\n'))
		return
	}
	...
	for _, p := range ports {
		if p == 0 {
			dynPort, err := s.FindFreeDynamicPort()
			if err != nil {
				s.logger.Error("Failed to allocate dynamic port", slog.String("client_id", clientID), slog.String("error", err.Error()))
				s.mu.Unlock()
				resp, _ := json.Marshal(AuthResponse{Status: "error", Error: "No dynamic ports available"})
				_, _ = conn.Write(append(resp, '\n'))
				return
			}
			finalPorts = append(finalPorts, dynPort)
			s.logger.Info("Dynamically allocated port for client", slog.String("client_id", clientID), slog.Int("allocated_port", dynPort))
		} else {
	...
	if _, err := conn.Write(append(respData, '\n')); err != nil {
		s.logger.Error("Failed to send auth response payload to client", slog.String("error", err.Error()))
		s.mu.Unlock()
		return
	}
	...
	s.logger.Info("Client authenticated successfully", slog.String("client_id", clientID), slog.String("remote_addr", conn.RemoteAddr().String()), slog.Any("assigned_ports", ports))
