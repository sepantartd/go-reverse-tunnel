...
func (s *TunnelServer) handleControlConnection(conn net.Conn) {
	...
	// 1. Generate secure random nonce (32 bytes)
	nonce := make([]byte, 32)
	...
	// Read client auth payload: "clientID:hmac_hex\n"
	buf := make([]byte, 1024)
	n, err := conn.Read(buf)
	if err != nil {
		return
	...
	// 2. Find client mapping in configuration
	s.mu.Lock()
	...
	if assignedClient == nil && len(s.config.Clients) > 0 {
		s.logger.Warn("Client ID not allowed/found in configuration", slog.String("client_id", clientID))
		s.mu.Unlock()
		return
	}
