...
func (c *ServerConfig) Validate() error {
	...
	if c.EnableAutoTLS {
		if c.AutoTLSDomain == "" {
			return fmt.Errorf("auto_tls_domain is required when enable_auto_tls is true")
		}
	} else if (c.TLSCertFile == "" || c.TLSKeyFile == "") && !c.InsecureAllowPlaintext {
		return fmt.Errorf("TLS configuration missing (TLSCertFile/TLSKeyFile or EnableAutoTLS); set InsecureAllowPlaintext=true to bypass explicitly")
	}
